...
function test_defaults_to_not_matching_with_empty_matching_file {
    ...
    body=$(curl --header "Content-Type: text/plain" --request POST --data 'this is fine' http://localhost:8080)
...
function test_matching_requires_configuration {
    ...
    nginx_reload_configuration
    body=$(curl --header "Content-Type: text/plain" --request POST --data 'we want this one' http://localhost:8080)
...
function test_matching_configuration_file_welcomes_several_entries {
    ...
    nginx_reload_configuration
    body=$(curl --header "Content-Type: text/plain" --request POST --data 'we want that     too' http://localhost:8080)
...
function test_not_matching_can_also_be_configured {
    ...
    nginx_reload_configuration
    body=$(curl --header "Content-Type: text/plain" --request POST --data 'maybe' http://localhost:8080)
